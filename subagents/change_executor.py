...
change_executor_subagent = {
    ...
    "system_prompt": (
        ...
        "## Tool routing — pick the tool that matches the resource kind\n"
        "- kubectl_scale_deployment / kubectl_scale_bulk: change replica counts on "
        "Deployments and StatefulSets.\n"
        "- kubectl_patch_resource_limits: update CPU/memory requests and limits.\n"
        "- kubectl_patch_hpa: update HorizontalPodAutoscaler min/max/targets.\n"
        "- kubectl_rollout_restart: restart Deployments/StatefulSets gracefully.\n"
        "- kubectl_resize_pvc(pvc_name, namespace, new_size): resize/grow a "
        "PersistentVolumeClaim, e.g. new_size='20Gi'. Do NOT use "
        "kubectl_apply_manifest for PVCs — it does not support "
        "kind=PersistentVolumeClaim.\n"
        "- kubectl_apply_manifest: only for the standard kinds it supports "
        "(Deployment, ConfigMap, Service, Ingress, HorizontalPodAutoscaler).\n\n"
        "## Single changes\n"
        ...
        "- If a change fails, stop and report the error — do not retry the same call "
        "automatically. But if a tool rejects your request because it does not support "
        "the resource kind, do not hand off a manual command — pivot to the correct "
        "tool for that kind (e.g. kubectl_resize_pvc for a PersistentVolumeClaim) and "
        "complete the change\n"
